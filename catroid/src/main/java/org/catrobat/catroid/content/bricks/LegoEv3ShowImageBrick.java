...
import org.catrobat.catroid.ui.dialogs.LegoImageContrastDialog;
...
public class LegoEv3ShowImageBrick extends SetLookBrick {
	...
	public void onLookDataListChangedAfterNew(LookData lookData) {
		...
		look.setLookRgf(convertLookDataToRgf());
	}
	...
	private byte[] convertLookDataToRgf() {
		...
		LegoImageContrastDialog dialog = new LegoImageContrastDialog(tmpBitmap, threshold);
		// listen to dialog threshold and set to this threshold
//		Log.d("ALT_D"," threshold: " + threshold);
		int centerX = (MAX_LEGO_IMAGE_WIDTH - imageWidth) / 2;
		int centerY = (MAX_LEGO_IMAGE_HEIGHT - imageHeight) / 2;
//		Log.d("ALT_D", "centerX=" + centerX + " centerY=" + centerY);
...
//		Log.d("ALT_D","width=" + bitmap.getWidth() + " height="+bitmap.getHeight());
		rgf[0] = (byte) MAX_LEGO_IMAGE_WIDTH;
		rgf[1] = (byte) MAX_LEGO_IMAGE_HEIGHT;
		int p, r, g, b, gray;
		int iPixel = 0, iByte = 1;
//		Log.d("ALT_D","width: " + imageWidth + " height: " + imageHeight +" h*w = " + imageHeight*imageWidth/8 +
